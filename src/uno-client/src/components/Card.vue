...
const cardClass = computed(() => {
  return props.color ? props.color.toLowerCase() : "wild"
})

function getCardImage(color: string | null, type: string | null, value?: number | null) {
  if (!color || !type) {
    return "Deck.png" // fallback hvis data mangler
  }
...
<template>
  <div class="card" :class="cardClass">
    <img class="card-img" :src="imageUrl" :alt="`${props.color ?? ''} ${props.type ?? ''}`" />
  </div>
</template>
